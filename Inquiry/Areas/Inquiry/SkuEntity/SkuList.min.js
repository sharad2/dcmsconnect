$(document).ready(function(){"use strict";$("#tbSkuId").typeahead(null,{name:"skus",displayKey:"label",source:function(n,t){var i=_autocompleteUrl.replace("~",n);$.get(i).done(function(n){this.cb(n)}.bind({cb:t})).fail(function(n,t,i){n.status==500?this.cb([{label:"Error "+(n.responseText||i),value:""}]):this.cb([{label:"Http Error "+n.status+": "+i+" "+this.url,value:""}])}.bind({cb:t,url:i}))},templates:{empty:"No matching SKUs found"}}).on("typeahead:selected typeahead:autocompleted",function(n,t){$("#hfSkuId").val(t.value);$(n.delegateTarget).closest("form").trigger("submit")}).on("input",function(){$("#hfSkuId").val("")})});
/*!
 * typeahead.js 0.10.5
